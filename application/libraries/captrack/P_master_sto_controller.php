...
class P_master_sto_controller {
    ...
    function html_select_options_org_path3() {
        try {
            ...
            $sql  = "select distinct(org_path3) from p_master_sto order by org_path3 asc";
            $query = $table->db->query($sql);
            $row = $query->result_array();

            echo '<select>';
            foreach($row  as $item ){
                echo '<option value="'.$item['org_path3'].'">'.$item['org_path3'].'</option>';
            }
            echo '</select>';
            exit;
        }catch (Exception $e) {
    ...
    function html_select_options_recode() {
        try {
            $ci = & get_instance();
            $ci->load->model('captrack/p_master_sto');
            $table = $ci->p_master_sto;
            
            $sql  = "select distinct(recode) from p_master_sto order by recode asc";
            $query = $table->db->query($sql);
            $row = $query->result_array();

            echo '<select>';
            foreach($row  as $item ){
                echo '<option value="'.$item['recode'].'">'.$item['recode'].'</option>';
            }
            echo '</select>';
            exit;
        }catch (Exception $e) {
